...
export class ShxFont {
  ...
  /**
   * Return true if this font contains glyph of the specified character.
   */
   ...
   * Uses legacy parsing with normalizeToOrigin for bigfont.
   ...
   * Uses inline subshape execution and preserves baseline information.
   * Does NOT normalizeToOrigin for bigfont.
   *
   * @param code - The character code
   * @param options - CAD options (fontSize, verticalText, etc.)
   * @returns The CAD shape result with proper baseline, bbox, and advance
   */
  public getCadCharShape(code: number, options: CadCharShapeOptions = {}): CadCharShapeResult | undefined {
    return this.shapeParser.executeCadShape(code, options);
  }
