    ...
    try {
      ...
      for (const item of items) {
        try {
          ...
          const bytes = reader.readBytes(item.length);
          if (bytes.length === item.length) {
            // BigFont shape data may include a null-terminated name/comment
            // before the actual bytecode. Skip it (same as unifont parser).
            // But shape 0 is the font info block — don't skip its data.
