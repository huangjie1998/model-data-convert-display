...
/**
 * Options for CAD-level character shape retrieval
 */
export interface CadCharShapeOptions {
  /** Whether to normalize bigfont glyphs to origin (default: false for CAD mode) */
  normalizeBigFont?: boolean;
  /** Whether the text is vertical (affects command 14 handling) */
...
/**
 * Result of a CAD-level character shape retrieval
 */
export interface CadCharShapeResult {
  /** Polylines defining the character shape */
  polylines: { x: number; y: number }[][];
  /** Last point of the shape (for advance calculation) */
  lastPoint: { x: number; y: number } | undefined;
  /** Bounding box of the shape */
  bbox: { minX: number; minY: number; maxX: number; maxY: number };
  /** Scale factor that was active at the end of execution */
  finalScale: number;
  /** Baseline Y position (always 0 in program coordinates) */
  baselineY: number;
}
