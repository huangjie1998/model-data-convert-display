...
export class MultiObserver<T> {
   ...
   */
  public static Watch<T>(observables: Observable<T>[], callback: (eventData: T, eventState: EventState) => void, mask = -1, scope: any = null): MultiObserver<T> {
...
export class Observable<T> {
   ...
   */
  public add(callback: (eventData: T, eventState: EventState) => void, mask = -1, insertFirst = false, scope: any = null, unregisterOnFirstCall = false): Nullable<Observer<T>> {
    if (!callback) {
      return null;
    }
   ...
   */
  public notifyObservers(eventData: T, mask = -1, target?: any, currentTarget?: any, userInfo?: any): boolean {
   ...
   */
  public notifyObserversWithPromise(eventData: T, mask = -1, target?: any, currentTarget?: any, userInfo?: any): Promise<T> {
   ...
   * @param mask is used to filter observers defaults to -1
   */
  public notifyObserver(observer: Observer<T>, eventData: T, mask = -1): void {
    if (observer._willBeUnregistered) {
      return;
    }
   ...
   **/
  public hasSpecificMask(mask = -1): boolean {
    for (const obs of this._observers) {
