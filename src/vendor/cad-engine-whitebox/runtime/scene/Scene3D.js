...
export class Scene3D {
  ...
  _recordTextBuildError(err, textPayload) {
    ...
    const text = String(textPayload?.text || '').slice(0, 80);
  ...
  _shxTextForDisplay(textPayload) {
    return normalizeCadTextForDisplay(textPayload?.text);
  }

  _shxCadShapeHasVisibleStrokes(cadShape) {
    const polylines = cadShape?.polylines;
  ...
  _shxGlyphAdvanceFromGlyph(glyph, raw) {
    ...
    if (!Number.isFinite(advancePointX) || advancePointX <= 0) {
      ...
      throw new Error(
        `SHX glyph has no advancePoint: char=${JSON.stringify(raw)} ` +
  ...
  _resolveShxGlyphByCode(code, textPayload, { requireVisible = false, recordMissing = true } = {}) {
    ...
    const entityFont = this._getShxFontByKey(textPayload?.fontKey) || this._shxFont;
  ...
  _shxUnitsToText(units) {
    return Array.isArray(units) ? units.map((unit) => unit?.raw || '').join('') : '';
  }
  ...
  _trimStartShxUnits(units) {
    ...
    while (out.length && /^\s+$/u.test(out[0]?.raw || '')) out.shift();
    return out;
  }

  _trimEndShxUnits(units) {
    const out = Array.isArray(units) ? [...units] : [];
    while (out.length && /^\s+$/u.test(out[out.length - 1]?.raw || '')) out.pop();
    return out;
  }
  ...
  _shxWrapUnitLineToWidth(units, textPayload, maxWidth) {
    ...
    for (const unit of rawUnits) {
      const isWhitespace = /^\s+$/u.test(unit?.raw || '');
    ...
    for (const token of tokens) {
      const isWhitespace = token.every((unit) => /^\s+$/u.test(unit?.raw || ''));
  ...
  _cadUnitsFallbackText(units) {
    return Array.isArray(units)
      ? units.map((unit) => unit?.escape === true ? '?' : unit?.raw || '').join('')
      : '';
  }
  ...
  async _loadShxFontByKey(fontKey) {
    ...
    try {
      ...
      this._shxFontByKey.delete(fontKey); // 加载失败，移除标记以允许下次重试
    }
  }
  ...
  _textPayloadUsesEntityShxFont(textPayload) {
    const fontKind = String(textPayload?.fontKind || '').toLowerCase();
    const fontName = String(textPayload?.fontName || '').toLowerCase();
    const bigFontName = String(textPayload?.bigFontName || '').toLowerCase();
    return fontKind === 'shx' || fontName.endsWith('.shx') || !!textPayload?.bigFontKey || bigFontName.endsWith('.shx');
  }
  ...
  _ensureTextPayloadFontsLoaded(textPayload) {
    ...
    ensure(textPayload?.fontKey);
    ensure(textPayload?.bigFontKey);
    return !pending;
  }
  ...
  _resolveShxGlyph(char, textPayload, recordMissing = true) {
    ...
    if (entityFont) {
      const isBigFont = entityFont.fontData?.header?.fontType === 'bigfont';
      if (isBigFont) {
        // BigFont entityFont：用 GB 翻译码查（半角 ASCII 时 _shxCodeForChar 退回 codePoint）
        ...
        // BigFont 单字节 ASCII 区通常没字形（如 ACCI-KT 0/96），让流程继续往下走主字体兜底
    ...
    // 1b. 文字实体专属的大字体（text_style.bigfont_name，如 ACCI-KT.SHX / gbcbig.shx）。
    // 跨实体共享一份 _shxBigFont 会让 "样式 1=wlc-e1+ACCI-KT" 的实体被
    // 全局 bigfont（来自最常用样式）的字形覆盖，必须先查实体自己的大字体。
    const entityBigFontKey = textPayload?.bigFontKey;
    ...
    // 3. 全局大字体（如 gbcbig.shx）— 兜底使用，仅当实体没指定 bigfont 或自家 bigfont 也没字形时。
    ...
    if (codePoint === 0x3000) {
      if (this._shxFont && this._shxFont.hasChar(0x20)) {
        const cadShape = this._shxFont.getCadCharShape(0x20, {
          fontSize: this._shxGlyphFontSizeForFont(textPayload, this._shxFont),
          verticalText: textPayload?.verticalText === true,
        });
        if (cadShape) {
          const shape = { polylines: cadShape.polylines, bbox: cadShape.bbox, lastPoint: cadShape.lastPoint };
          return { shape, source: 'font', code: 0x20 };
        }
      }
      if (entityFont && entityFont.hasChar(0x20)) {
        const cadShape = entityFont.getCadCharShape(0x20, {
          fontSize: this._shxGlyphFontSizeForFont(textPayload, entityFont),
          verticalText: textPayload?.verticalText === true,
        });
        if (cadShape) {
          const shape = { polylines: cadShape.polylines, bbox: cadShape.bbox, lastPoint: cadShape.lastPoint };
          return { shape, source: 'entity_font', code: 0x20 };
        }
      }
    }
  ...
  _shxGlyphFontSizeForFont(textPayload, font) {
    ...
    if (textPayload?.isMText === true) return height;
    const content = font?.fontData?.content;
    if (font?.fontData?.header?.fontType === 'bigfont') return height;
    const cellH = toFiniteNumber(content?.height, 0);
    const baseUp = toFiniteNumber(content?.baseUp, 0);
    if (cellH > 0 && baseUp > 0) {
  ...
  _shxGlyphFontSize_text(textPayload, glyphSource) {
    return Math.max(1e-6, toFiniteNumber(textPayload?.height, 1));
  }
  ...
  _shxTextLineAdvance(textOrUnits, textPayload, recordMissing = false) {
    ...
    for (const unit of units) {
      advance += toFiniteNumber(this._resolveShxUnit(unit, textPayload, recordMissing)?.advance, 0);
    }
    return advance * widthFactor;
  }
  ...
  _buildShxStrokeLineGeometry(lines, textPayload, advanceFn) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      for (const unit of units) {
        ...
        const advance = toFiniteNumber(resolved?.advance, 0);
  ...
  _buildTextObject(textPayload) {
    ...
    let shxObject = null;
    try {
      shxObject = textPayload?.isMText === true
  ...
  async _applyParsedBundle(glxJson, meshArrayBuffer) {
    ...
    // 在构建几何之前，先把所有实体引用到的 SHX font_key / bigfont_key 加载齐全。
    // _buildShxStrokeTextGeometry_* 是同步的：第一次构建后 _resolveShxGlyph 会
    // 把字形烘进 THREE.LineSegments；后续 _loadShxFontByKey 完成只把
    // needsUpdateGPUData=true，这只触发 render()，不会重建几何，也不会改用
    // entity 自己的 bigfont（如 ACCI-KT.SHX/wlc-e1.shx）。所以必须在构建前
    // 等所有 per-entity SHX 加载完。
    const fontKeysToLoad = new Set();
    for (let li = 0; li < parsed.layers.length; li += 1) {
      const items = parsed.layers[li]?.items || [];
      for (let ji = 0; ji < items.length; ji += 1) {
        const it = items[ji];
        if (it?.kind !== 'text' || !it.payload) continue;
    ...
    if (fontKeysToLoad.size > 0) {
      await Promise.all(
        Array.from(fontKeysToLoad).map((k) => this._loadShxFontByKey(k).catch(() => undefined))
      );
    }
