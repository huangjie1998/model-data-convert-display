...
def build_attdef_entity(state: Dict[str, object], context) -> Dict[str, object] | None | object:
    if state.get("et") != "acdbattributedefinition":
        return NOT_HANDLED
    return _build_attribute_text_entity(state, context, "ATTDEF", "ACDBATTRIBUTEDEFINITION")


def _build_attribute_text_entity(state: Dict[str, object], context, entity_type: str, source_type: str) -> Dict[str, object] | None:
